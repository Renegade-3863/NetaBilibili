...
static int acceptConnection(void *arg)
{
    ...
    while (1)
    {
        ...
        /* Enable TCP keepalive so the kernel can detect dead peers and free ESTABLISHED sockets. */
        {
            int keepalive = 1;
            setsockopt(cfd, SOL_SOCKET, SO_KEEPALIVE, &keepalive, sizeof(keepalive));
            int idle = 5;       /* start keepalive probes after 5s idle */
            int interval = 5;    /* interval between probes */
            int probes = 3;      /* number of probes before considered dead */
